...
server_null_alleles <- function(id, rv) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns  # IMPORTANT: stocker ns pour l'utiliser dans renderUI
  ...
  FROM %s GROUP BY CAST(%s AS VARCHAR))", hl_q, hf_tbl_q, hl_q)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  EM ALGORITHM CORRIGÉ - Version Pascal FreeNA exacte
    # ══════════════════════════════════════════════════════════════════════════
    em_freena <- function(gt_vec, base, treat = "absent") {
      ...
      # Convertir en character pour les noms
      alleles <- as.character(all_alleles)
      names(alleles) <- alleles
      
      N_total <- efpop - n_absent  # Nombre total d'individus (inclut null_homo si traités comme génotypes)
      N_valid <- N_total - n_null_homo  # Nombre de génotypes non-nuls
      ...
      # Initialisation de rd (fréquence de l'allèle nul)
      ...
      # Initialisation des fréquences alléliques corrigées
      ...
      for (iter in seq_len(5000L)) {
        ...
        p <- new_p
        rd <- max(0.0, min(1.0, rd_new))  # Borner rd entre 0 et 1
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  MAIN COMPUTATIONS (functions identiques à avant, garder les mêmes)
    # ══════════════════════════════════════════════════════════════════════════
      ...
      for (li in seq_along(markers)) {
        ...
        for (a in alleles_obs) {
          ...
          pop_raw <- lapply(pops, function(p) {
            e <- em_loc[[p]]; ni <- max(0L,e$N - e$n_null_homo)
          ...
          pop_ena <- lapply(pops, function(p) {
            e <- em_loc[[p]]; ni <- max(0L,e$N - e$n_absent)
    ...
    results_r <- eventReactive(input$run_all, {
      ...
      withProgress(message = "Running computations...", value = 0, {
        ...
        for (loc in markers) {
          for (pop in pops) {
            ...
            t1_rows[[length(t1_rows)+1L]] <- data.frame(
              ...
              N = e$N,
              N_exp_blanks = round(e$N * (e$rd^2), 6),
              stringsAsFactors = FALSE
            )
          }
        }
        ...
        # Table 2: Global weighted mean per locus
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  FILE DOWNLOADS - CORRIGÉS avec ns()
    # ══════════════════════════════════════════════════════════════════════════
    ...
    output$dl_file1_csv <- downloadHandler(
      ...
      content  = function(file) {
        ...
        hdr <- c(hdr, "# Section 1: p_nulls per locus x population", "#")
        write_with_header(hdr, r$t1, file, sep = ",")
        write("", file = file, append = TRUE)
        write("# Section 2: N-weighted mean per locus (all populations)",
              file = file, append = TRUE)
        write.table(r$t2, file = file, sep = ",", row.names = FALSE,
    ...
    # ── Download buttons UI — CORRIGÉS avec ns() ────────────────────────────────
    ...
    output$vb_n <- renderUI({
      tryCatch({
        ...
        n <- DBI::dbGetQuery(con, sprintf(
          ...
          sql_id(con, ms$ind_col), sql_id(con, tbl_meta_r()), sql_id(con, ms$ind_col)
        ))$n[[1]]
        tags$span(n)
    ...
    output$dt_t1 <- DT::renderDT({
      ...
      d <- r$t1
      names(d) <- c("Locus", "Population", "Coding", "p_nulls", "N", "N_exp_blanks")
    ...
    output$dt_t2 <- DT::renderDT({
      ...
      d <- r$t2
      names(d) <- c("Locus", "Coding", "Av(p_nulls)", "N_tot")
      DT::datatable(d, rownames = FALSE,
        options = list(pageLength = 20, scrollX = TRUE, dom = "lftip",
