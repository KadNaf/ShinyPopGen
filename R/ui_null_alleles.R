  ...
  tags$div(class="na-module", custom_css,
    ...
    tags$div(class="na-vbox-row",
      ...
      tags$div(class="na-vbox",
        ...
        tags$div(tags$div(class="na-vbox-label","Max p_nulls"), tags$div(class="na-vbox-val",uiOutput(ns("vb_max_null"))))),
      tags$div(class="na-vbox",
    ...
    tags$div(class="na-panel",
      tags$div(class="na-panel-head",
        tags$div(class="na-panel-title", icon("sliders-h"), " Setup \u2014 3 parameters to configure")),
      tags$div(class="na-panel-body",
        ...
        tags$br(), tags$br(),
        fluidRow(
          column(4, numericInput(ns("nboot"), label = "Number of replicates:", value = 5000, min = 100, max = 99999, step = 1000)),
          column(4, selectInput(ns("ci_level"), label = "Confidence interval level:",
            choices = c("99.99% (alpha = 0.0001)" = "0.0001", "99.9% (alpha = 0.001)" = "0.001",
                        "99% (alpha = 0.01)" = "0.01", "95% (alpha = 0.05)" = "0.05",
                        "90% (alpha = 0.10)" = "0.10"), selected = "0.05")),
          column(4, tags$div(style="margin-top:25px;font-size:11px;color:#64748b;",
            icon("info-circle"), " Bootstrap over loci: vectorised, fast.",
            tags$br(), "Bootstrap over sub-samples: re-runs EM per replicate."))
        ),

        tags$hr(style="margin:1rem 0;"),

        tags$strong("(3) Run all computations + generate output files", style="font-size:12px; color:#1e293b;"),
        tags$br(), tags$br(),
        fluidRow(column(4, actionButton(ns("run_all"), label = tagList(icon("play"), tags$strong(" Compute + Bootstrap + Export")),
          class = "na-btn-run btn", width = "100%"))),
        br(),
    ...
    tags$div(class="na-panel",
      tags$div(class="na-panel-head",
        tags$div(class="na-panel-title", icon("file-download"), " Output files \u2014 automatically generated after computation")),
      tags$div(class="na-panel-body",
        tags$div(class="na-info", icon("info-circle"), " All four files are generated automatically when you click Compute above."),
        fluidRow(
          column(3, tags$div(class="na-panel", style="border-color:#bfdbfe;",
            tags$div(class="na-panel-head", style="background:#eff6ff;",
              tags$div(class="na-panel-title", style="color:#1d4ed8;", icon("file-alt"), " File 1 \u2014 Null allele frequencies")),
            tags$div(class="na-panel-body", style="font-size:11px;color:#334155;",
              "p_nulls per locus \u00d7 population", tags$br(), "Global weighted mean per locus",
              tags$br(), "Locus coding reminder", tags$br(), br(), uiOutput(ns("ui_dl_file1"))))),
          column(3, tags$div(class="na-panel", style="border-color:#99f6e4;",
            tags$div(class="na-panel-head", style="background:#f0fdfa;",
              tags$div(class="na-panel-title", style="color:#0d9488;", icon("chart-bar"), " File 2 \u2014 Global FST & FST-ENA")),
            tags$div(class="na-panel-body", style="font-size:11px;color:#334155;",
              "Per locus + multilocus FST / FST-ENA", tags$br(), "CI from bootstrap over loci",
              tags$br(), "CI from bootstrap over sub-samples", uiOutput(ns("ui_dl_file2"))))),
          column(3, tags$div(class="na-panel", style="border-color:#e9d5ff;",
            tags$div(class="na-panel-head", style="background:#faf5ff;",
              tags$div(class="na-panel-title", style="color:#7c3aed;", icon("table"), " File 3 \u2014 Pairwise long format")),
            tags$div(class="na-panel-body", style="font-size:11px;color:#334155;",
              "FST, FST-ENA, DCSE, DCSE-INA", tags$br(), "Per pair of sub-samples",
              tags$br(), "CI from bootstrap over loci", uiOutput(ns("ui_dl_file3"))))),
          column(3, tags$div(class="na-panel", style="border-color:#fcd34d;",
            tags$div(class="na-panel-head", style="background:#fffbeb;",
              tags$div(class="na-panel-title", style="color:#92400e;", icon("th"), " File 4 \u2014 Per-locus half-matrices")),
            tags$div(class="na-panel-body", style="font-size:11px;color:#334155;",
              "FST, FST-ENA, DCSE, DCSE-INA", tags$br(), "Half-matrix per locus",
    ...
    tabsetPanel(id = ns("na_tabs"), type = "tabs",
      ...
      tabPanel(title = tagList(icon("dna"), " Null allele frequencies"), value = "tab_na", br(),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("list"), " p_nulls per locus \u00d7 population (EM algorithm)")),
          tags$div(class="na-panel-body", DT::DTOutput(ns("dt_t1")))),
        tags$br(),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("globe"), " Global summary per locus (N-weighted mean)")),
          tags$div(class="na-panel-body", DT::DTOutput(ns("dt_t2"))))
      ),

      tabPanel(title = tagList(icon("chart-bar"), " FST / FST-ENA"), value = "tab_fst", br(),
        tags$div(class="na-info",
          icon("info-circle"), " ", tags$strong("Global multilocus FST"), " \u2014 Weir (1996) / Genepop method. ",
          tags$strong("FST-ENA"), ": EM-corrected frequencies, Excluding Null Alleles."),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("list"), " Per-locus FST and FST-ENA")),
          tags$div(class="na-panel-body", DT::DTOutput(ns("dt_fst_global")))),
        br(),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("random"), " Bootstrap CI \u2014 Global FST and FST-ENA")),
          tags$div(class="na-panel-body", uiOutput(ns("ui_boot_global_fst")))),
        br(),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("exchange-alt"), " Pairwise FST and FST-ENA")),
          tags$div(class="na-panel-body",
            fluidRow(column(5, radioButtons(ns("fst_pair_display"), "Display:",
              choices = c("Raw FST" = "raw", "FST-ENA" = "ena", "Both" = "both"), selected = "both", inline = TRUE))),
            uiOutput(ns("ui_fst_pair_matrix")))),
        br(),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("random"), " Bootstrap CI \u2014 Pairwise FST-ENA")),
          tags$div(class="na-panel-body", uiOutput(ns("ui_boot_pair_fst"))))
      ),

      tabPanel(title = tagList(icon("ruler-combined"), " DCSE / DCSE-INA"), value = "tab_dc", br(),
        tags$div(class="na-info",
          icon("info-circle"), " ", tags$strong("Cavalli-Sforza & Edwards (1967) chord distance."),
          " DCSE-INA includes the null allele as an extra state."),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("th"), " Pairwise DCSE and DCSE-INA")),
          tags$div(class="na-panel-body",
            fluidRow(column(5, radioButtons(ns("dc_display"), "Display:",
              choices = c("Raw DCSE" = "raw", "DCSE-INA" = "ina", "Both" = "both"), selected = "both", inline = TRUE))),
            uiOutput(ns("ui_dc_matrix")))),
        br(),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("random"), " Bootstrap CI \u2014 Pairwise DCSE-INA")),
          tags$div(class="na-panel-body", uiOutput(ns("ui_boot_pair_dc"))))
      ),

      tabPanel(title = tagList(icon("table"), " Per-locus \u00d7 pair"), value = "tab_locus_pair", br(),
        tags$div(class="na-info", icon("info-circle"), " FST, FST-ENA, DCSE and DCSE-INA for each locus \u00d7 pair of populations."),
        fluidRow(
        ...
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("list"), " FST and FST-ENA per locus \u00d7 pair")),
          tags$div(class="na-panel-body", DT::DTOutput(ns("dt_fst_locus")))),
        br(),
        tags$div(class="na-panel",
          tags$div(class="na-panel-head", tags$div(class="na-panel-title", icon("list"), " DCSE and DCSE-INA per locus \u00d7 pair")),
          tags$div(class="na-panel-body", DT::DTOutput(ns("dt_dc_locus"))))
      )
    )
  )
}